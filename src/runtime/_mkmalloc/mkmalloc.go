...
var noos = flag.Int("noos", 0, "0 - generate OS version, 32 or 64: generate noos version")
...
func pathNoos(p string) string {
	ret, _ := strings.CutSuffix(p, ".go")
	switch *noos {
	case 32:
		ret += "_noos32.go"
	case 64:
...
func main() {
	...
	sizeclasesesfile := pathNoos("../../internal/runtime/gc/sizeclasses.go")
	if err := os.WriteFile(sizeclasesesfile, mustFormat(generateSizeClasses(classes)), 0666); err != nil {
		log.Fatal(err)
	}

	outfile := pathNoos("../malloc_generated.go")
...
func inline(config generatorConfig) []byte {
	...
	out.WriteString("//go:build " + build + "\n\n")
