...
// R0. It can clobber R0-R4,LR registers (other registers should be preserved).
...
#define SYS_MAX_ARGS (24+4) // max. size of argumants and return values
...
// called almost like normal function so it does not have to save any registers
// except g (R10). It saves more (g, PSP, LR, CONTROL[nPRIV]) to be ready for
// context switch.
TEXT runtime·svcallHandler(SB),NOSPLIT|NOFRAME,$0-0
	...
	CMP  $SYS_NUM, R4
	BGE  ·badSyscall(SB)  // does not return
...
// wakeup threads waiting for some external event). The scheduler can clear
// PendSV pending bit before do its work to avoid reentry after return. The
// interrupt handler that wants to signal some external event to the thread that
// wait for it on the certain futex, wakes this thread and generates PendSV
// exception to run the scheduler. To avoid overlooking of this exception (the
// scheduler can clear pending bit) the interrupt handler must use DMB
// instruction to ensure that the PendSV pending state is not observable before
// the memory accesses that was performed to wakeup the thread.
