...
func ssaGenValue(s *ssagen.State, v *ssa.Value) {
	...
	case ssa.OpThumbLoweredPanicBoundsRR, ssa.OpThumbLoweredPanicBoundsRC, ssa.OpThumbLoweredPanicBoundsCR, ssa.OpThumbLoweredPanicBoundsCC,
		ssa.OpThumbLoweredPanicExtendRR, ssa.OpThumbLoweredPanicExtendRC:
		...
		extend := false
		switch v.Op {
		case ssa.OpThumbLoweredPanicBoundsRR:
			xIsReg = true
			xVal = int(v.Args[0].Reg() - thumb.REG_R0)
			yIsReg = true
			yVal = int(v.Args[1].Reg() - thumb.REG_R0)
		case ssa.OpThumbLoweredPanicExtendRR:
			extend = true
			xIsReg = true
			hi := int(v.Args[0].Reg() - thumb.REG_R0)
			lo := int(v.Args[1].Reg() - thumb.REG_R0)
			xVal = hi<<2 + lo // encode 2 register numbers
			yIsReg = true
			yVal = int(v.Args[2].Reg() - thumb.REG_R0)
		case ssa.OpThumbLoweredPanicBoundsRC:
			xIsReg = true
			xVal = int(v.Args[0].Reg() - thumb.REG_R0)
			...
			if c >= 0 && c <= abi.BoundsMaxConst {
				...
				p.From.Offset = c
				p.To.Type = obj.TYPE_REG
				p.To.Reg = thumb.REG_R0 + int16(yVal)
			}
		case ssa.OpThumbLoweredPanicExtendRC:
			extend = true
			xIsReg = true
			hi := int(v.Args[0].Reg() - thumb.REG_R0)
			lo := int(v.Args[1].Reg() - thumb.REG_R0)
		...
		case ssa.OpThumbLoweredPanicBoundsCR:
			yIsReg = true
			yVal = int(v.Args[0].Reg() - thumb.REG_R0)
			c := v.Aux.(ssa.PanicBoundsC).C
			if c >= 0 && c <= abi.BoundsMaxConst {
				xVal = int(c)
				...
				p.From.Offset = c
				p.To.Type = obj.TYPE_REG
				p.To.Reg = thumb.REG_R0 + int16(xVal)
			} else {
				...
				p.From.Offset = c >> 32
				p.To.Type = obj.TYPE_REG
				p.To.Reg = thumb.REG_R0 + int16(hi)
				p = s.Prog(thumb.AMOVW)
				p.From.Type = obj.TYPE_CONST
				p.From.Offset = int64(int32(c))
				p.To.Type = obj.TYPE_REG
				p.To.Reg = thumb.REG_R0 + int16(lo)
			}
		case ssa.OpThumbLoweredPanicBoundsCC:
